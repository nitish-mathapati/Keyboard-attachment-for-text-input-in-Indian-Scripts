...
void process_kbd_attachment(uint8_t *keyinfo,hid_keyboard_report_t *kbdreport)
{
    ...
    //make it 0 to not send any keypress at all

    //making the current list of variables zero
    listlen=0;
...
static void keymod(uint8_t *keylist,uint8_t *prevkeylist,uint8_t *listlen,uint8_t *prevlistlen,uint8_t *sendkeypress,uint8_t *keyinfo)
{
    ...
    else if(*listlen==2) //this block exeutes if a key is pressed while holding another key
    {
        ...
        if((*(*(dualkeylist+i)+1))==keylist[0]||(*(*(dualkeylist+i)+2))==keylist[0])
        {
        if((*(*(dualkeylist+i)+1)==keylist[1])||(*(*(dualkeylist+i)+2))==keylist[1])
        {
            unicode_keypress(chnumtouc[*(*(dualkeylist+i)+0)],keyinfo); //i'th row 0'th column
            *sendkeypress=0;
            break;
        }
        }
